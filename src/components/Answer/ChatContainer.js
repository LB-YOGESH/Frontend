...
const ChatContainer = ({
  ...
  const upVoteAnswer = async (answer) => {
    let up, down;
    ApiGet(`answer/id=${answer?._id}`)
      .then((res) => {
        up = res.data.payload.singleAnswer.upVote;
        down = res.data.payload.singleAnswer.downVote;
      })
  ...
  const downVoteAnswer = async (answer) => {
    ...
    ApiGet(`answer/vote/id=${answer?._id}`)
      .then((res) => {
        ...
        ApiPut(
          `answer/downVote/id=${answer?._id}&downVote=${down}&upVote=${up}`
        )
          .then((res) => {
            if (res.data.result === 0) {
              // fetchQuestionIHaveReceived(true);
